...
export default function ChangelogModal({
  ...
  return (
    ...
    >
      <div
        className="max-w-2xl w-full max-h-[min(85vh,720px)] flex flex-col rounded-xl border border-[var(--color-ds-border)] bg-[var(--color-ds-surface)] shadow-[0_24px_64px_rgba(0,0,0,0.55)]"
        onClick={(e) => e.stopPropagation()}
      >
        <header className="shrink-0 flex items-center justify-between gap-4 px-5 py-4 border-b border-[var(--color-ds-border)]">
          <h2 id="changelog-modal-title" className="text-sm font-semibold text-[var(--color-ds-text)]">
            {title}
          </h2>
          <button
            type="button"
          ...
          >
            <svg viewBox="0 0 16 16" className="w-4 h-4 fill-none stroke-current stroke-[1.8]" strokeLinecap="round">
        ...
        <div className="flex-1 min-h-0 overflow-y-auto px-5 py-4">
          {loading && (
            <p className="text-sm text-[var(--color-ds-muted)]">{loadingLabel}</p>
          )}
